...
import { FONT_CROWN, FONT_UI } from '../assets/fonts';
import type { Ctx } from '../core/game';

/** Cinzel display text (headers, titles). */
export function displayText(text: string, size: number, color: number = CONFIG.colors.ink, weight: '700' | '900' = '700'): Text {
  const t = new Text({
    text,
    style: {
      fontFamily: FONT_CROWN, fontSize: size, fontWeight: weight, fill: color,
...
/** Inter body/UI text. */
export function uiText(text: string, size: number, color: number = CONFIG.colors.ink, weight: '400' | '600' | '800' = '400', wrap = 0): Text {
  const t = new Text({
    text,
    style: {
...
export class Bar extends Container {
  ...
  constructor(private w: number, private h: number, private color: number, backAlpha = 0.55) {
    ...
    back.roundRect(-w / 2, -h / 2, w, h, h / 2).fill({ color: CONFIG.colors.bg, alpha: backAlpha });
    back.roundRect(-w / 2, -h / 2, w, h, h / 2).stroke({ color: CONFIG.colors.inkDim, width: 1.5, alpha: 0.8 });
...
export class StarsRow extends Container {
  constructor(ctx: Ctx, earned: number, size: number, gap = 6) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      s.y = i === 1 ? -size * 0.14 : 0;
      s.tint = i < earned ? CONFIG.colors.gold : CONFIG.colors.panel;
      this.addChild(s);
    }
  }
}
...
export class Toggle extends Container {
  ...
  constructor(ctx: Ctx, public value: boolean, private onChange: (v: boolean) => void) {
    super();
    const w = 64, h = 34;
    this.track.roundRect(-w / 2, -h / 2, w, h, h / 2).fill(CONFIG.colors.panel)
      .stroke({ color: CONFIG.colors.inkDim, width: 2 });
  ...
  private draw(): void {
    ...
    this.knob.circle(this.value ? 15 : -15, 0, 12.5)
      .fill(this.value ? CONFIG.colors.gold : CONFIG.colors.inkDim);
...
export class HSlider extends Container {
  ...
  constructor(
    ...
    track.roundRect(-w / 2, -5, w, 10, 5).fill(CONFIG.colors.panel)
      .stroke({ color: CONFIG.colors.inkDim, width: 2 });
    this.addChild(track, this.fill, this.knob);
    this.knob.circle(0, 0, 15).fill(CONFIG.colors.ink).stroke({ color: CONFIG.colors.warn, width: 3 });
