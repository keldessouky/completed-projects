#!/usr/bin/env node
/**
 * Build Ziggurat Run as ONE self-contained .html file.
 *
 ...
 * handed to the boot scene through `window.__ZR_AUDIO__`.
 *
...
const OUT = args[0] ?? join(ROOT, 'dist-single', EMBED ? 'ziggurat-run.embed.html' : 'ziggurat-run.html');
...
(function () {
  ...
  window.__ZR_AUDIO__ = URL.createObjectURL(new Blob([bytes], { type: 'audio/wav' }));
})();
</script>`;
