#!/usr/bin/env node
/**
 * Ziggurat Run — procedural audio pipeline.
 *
...
/** inharmonic metallic strike (lamassu shields, boss armor) */
...
sfx.gateGood = (() => { // rising glazed-brick chime: D5 → A5
  const b = buf(0.4);
...
sfx.squadGain = (() => { // quick harp gliss up
  const b = buf(0.42);
...
sfx.lamassuWake = (() => { // stone grinding awake
...
const chunks = [];
const loops = new Set(['musicTitle', 'musicRun']);
