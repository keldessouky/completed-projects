...
export class Btn extends Container {
  ...
  constructor(private ctx: Ctx, private opts: BtnOpts) {
    super();
    const frame = opts.kind === 'gold' ? 'btnGold' : opts.kind === 'blue' ? 'btnBlue' : 'panelDark';
    ...
    if (opts.icon) {
      ...
      ic.anchor.set(0.5);
      ic.tint = opts.iconTint ?? CONFIG.colors.bone;
    ...
    if (opts.label) {
      const t = new Text({
        ...
        style: {
          ...
          fontWeight: '600',
          fill: opts.labelColor ?? (opts.kind === 'gold' ? CONFIG.colors.bitumen : CONFIG.colors.bone),
          align: 'center',
        },
      });
      t.anchor.set(0.5);
