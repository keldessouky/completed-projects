...
function dim(alpha = 0.62): Graphics {
  ...
  g.rect(-60, -60, W + 120, H + 120).fill({ color: CONFIG.colors.bitumen, alpha });
  g.eventMode = 'static';
  return g;
}
...
export function showSettings(ctx: Ctx): void {
  ...
  const sfx = new HSlider(ctx, 180, s.sfx, (v) => { s.sfx = v; ctx.applySettings(); }, () => ctx.audio.play('gateGood'));
