...
export class DevOverlay {
  ...
  private build(): void {
    ...
    // stage jump grid
    for (let i = 0; i < CONFIG.stages.count; i++) {
      const b = new Btn(ctx, {
        w: 56, h: 40, kind: 'blue', label: String(i + 1), labelSize: 15, silent: true,
        onTap: () => { this.toggle(false); ctx.router.goto('run', { stage: i }); },
      });
    ...
    const coins = new Btn(ctx, {
      w: 120, h: 44, kind: 'dark', label: '+500 ¢', labelSize: 14, silent: true,
      onTap: () => { ctx.save.data.coins += 500; ctx.save.mark(); },
    });
