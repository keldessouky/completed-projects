...
export class Bar extends Container {
  ...
  constructor(private w: number, private h: number, private color: number, backAlpha = 0.55) {
    ...
    back.roundRect(-w / 2, -h / 2, w, h, h / 2).fill({ color: CONFIG.colors.bitumen, alpha: backAlpha });
...
export class StarsRow extends Container {
  constructor(ctx: Ctx, earned: number, size: number, gap = 6) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      s.y = i === 1 ? -size * 0.14 : 0;
      s.tint = i < earned ? CONFIG.colors.gold : CONFIG.colors.starEmpty;
      this.addChild(s);
    }
  }
}
...
export class Toggle extends Container {
  ...
  constructor(ctx: Ctx, public value: boolean, private onChange: (v: boolean) => void) {
    super();
    const w = 64, h = 34;
    this.track.roundRect(-w / 2, -h / 2, w, h, h / 2).fill(CONFIG.colors.bitumenLift)
...
export class HSlider extends Container {
  ...
  constructor(
    ...
    track.roundRect(-w / 2, -5, w, 10, 5).fill(CONFIG.colors.bitumenLift)
      .stroke({ color: CONFIG.colors.boneDim, width: 2 });
    this.addChild(track, this.fill, this.knob);
    this.knob.circle(0, 0, 15).fill(CONFIG.colors.bone).stroke({ color: CONFIG.colors.ochreDeep, width: 3 });
