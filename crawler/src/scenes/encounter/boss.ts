...
/**
 * The floor's set piece: a blocking hulk with an HP pool and two arms that
 * wake at 50% and start cycling a shield across its face.
 * Damage-per-second is simply the party's arrow stream — surviving members
 * are the weapon. Breach = win; the hulk grinding down to the line = wipe.
 */
export class BossFight {
  phase: BossPhase = 'inactive';
  hp = 1;
  maxHp = 1;
  /** bottom edge of the hulk in design y — the crush line chases this */
  gateY = -320;
  awake = false;
  shielded = false;
  ...
  constructor(
    ...
    this.maxHp = this.hp = Math.max(1, bossHp);
    const a = ctx.atlas;
    this.gate = new Sprite(a.get('bossHulk'));
    this.gate.anchor.set(0.5, 1);
    this.armL = new Sprite(a.get('armIdle'));
    this.armL.anchor.set(0.5, 1);
    this.armL.scale.x = -1; // faces inward from the left flank
    this.armR = new Sprite(a.get('armIdle'));
    this.armR.anchor.set(0.5, 1);
    this.ring = new Sprite(a.get('shieldRing'));
    this.ring.anchor.set(0.5);
    this.ring.tint = CONFIG.colors.sysBright;
    this.ring.blendMode = 'add';
    this.ring.alpha = 0;
    this.view.addChild(this.gate, this.armL, this.armR, this.ring);
  ...
  tryHit(x: number, y: number, dmg: number): HitResult {
    ...
    if (this.shielded) {
      this.particles.burst(x, this.gateY - 30, {
        frame: 'spark', count: 3, tint: CONFIG.colors.sysBright, speed: 90, ttl: 0.25, additive: true, s0: 1, s1: 0.4,
      });
    ...
    this.particles.burst(x, y, {
      frame: 'shard', count: CONFIG.fx.hitParticles, tint: CONFIG.colors.amber, speed: 130, ttl: 0.35, gravity: 500,
    });
    this.pops.spawn(x, y - 18, String(Math.max(1, Math.round(dmg))), CONFIG.colors.amberBright);

    if (!this.awake && this.hpFrac <= CONFIG.boss.armsWakeAt) this.wake();
  ...
  private wake(): void {
    ...
    this.armL.texture = a.get('armAwake');
    this.armR.texture = a.get('armAwake');
    this.ctx.audio.play('bossWake');
    ...
    for (const lx of [this.cx - 150, this.cx + 150]) {
      this.particles.burst(lx, this.gateY - 60, {
        frame: 'softDot', count: 10, tint: CONFIG.colors.amberBright, speed: 70, ttl: 0.6, additive: true, s0: 1.4, s1: 0.2,
      });
    }
  }
  ...
  private breach(): void {
    ...
    // debris collapse: plating shears off the face, the arms drop
    const gy = this.gateY;
    for (let i = 0; i < 30; i++) {
      this.particles.burst(this.cx + (Math.random() - 0.5) * CONFIG.boss.gateWidth, gy - 40 - Math.random() * 180, {
        frame: 'brickChunk', count: 1,
        tint: i % 3 === 0 ? CONFIG.colors.amber : i % 2 === 0 ? CONFIG.colors.sysBright : CONFIG.colors.boneDim,
  ...
  frame(dtReal: number): void {
    ...
    if (this.phase !== 'breached') {
      this.armL.position.set(this.cx - 150, this.gateY);
      this.armR.position.set(this.cx + 150, this.gateY);
      // awake arms breathe
      if (this.awake) {
        const b = 1 + Math.sin(performance.now() / 300) * 0.015;
        this.armL.scale.set(-b, b);
        this.armR.scale.set(b, b);
      }
    }
