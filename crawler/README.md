...
- `pixelArt: true` samples that sheet with nearest-neighbour instead of
  bilinear. Set it at the top for a whole manifest, or per entry. **Turn it on
  for pixel art**, which is what nearly all downloadable sprite art is: a 32 px
  character drawn at 128 px through a bilinear filter is a smear, and through
  nearest it is what the artist drew at four times the size. Leave it off for
  smooth or painted art, where nearest gives you jagged edges for nothing.
...
A sprite pack off itch.io, OpenGameArt or the Kenney site is already a
pixel-exact grid of equal cells — nothing needs detecting, it needs *slicing*.
Tell the importer the grid and what its rows are:

```bash
npm run art:import -- Soldier.png --kind grunt --cell 52x64 \
  --grid 4x4 --map down,left,right,up --pixel --preview
```

- `--grid CxR` — the source is C columns × R rows of equal cells.
- `--map …` — what the source's rows **are**, top to bottom. Almost every pack
  is `down,left,right,up`; open the PNG and count before you guess. Accepted
  names: `down|south|s`, `up|north|n`, `left|west|w`, `right|east|e`,
  `downright|se`, `downleft|sw`, `upright|ne`, `upleft|nw`, and `skip` for a
  row that is not a facing at all (an idle strip, a death strip).
- `--pixel` — nearest-neighbour, and the scale is snapped to a whole number.
- `--walk 0,1,2,3` and `--attack 4` — which source columns are the four walk
  frames and the attack pose. The defaults handle the common shapes: a
  four-frame walk maps straight across, a three-frame walk becomes
  contact-passing-contact-passing, and with no fifth column the attack pose
  falls back to frame 0.

Three things it does on your behalf, worth knowing about:

**It maps the pack's four directions onto this game's five facings.** The game
paints `s, se, e, ne, n` and mirrors for the west half. A four-direction pack
has no diagonals, so `se` borrows `s` and `ne` borrows `n`. If a pack has only
a *left*-facing row and no right, that row is used **flipped** — the report line
says so:

```
rows        s←0(s)  se←0(s)  e←1(w flipped)  ne←3(n)  n←3(n)
```

**It crops every frame with one shared rectangle.** Trimming each frame to its
own content would be the obvious move and is wrong: a walk cycle's frames
differ in height by a pixel or two, and normalising each independently makes
the character pulse and hop through its own animation. The artist already
aligned those frames; the shared box keeps that and only trims the dead margin
they all share.

**It leaves an alpha channel alone.** If the PNG already has transparency —
packs nearly always do — transparent *is* the background, exactly, and no
colour keying happens. Guessing at a background colour there could only lose a
character their shirt for matching an unused corner.

Sizes: your source cell does not have to match the `--cell` you are filling.
A 32×32 source into `--cell 52x64` lands at ×6 with `--pixel` — chunky, and
consistent with itself, which is what matters. If you want it *less* chunky,
import at a lower `--scale` rather than fighting the fit.

One expectation to set before you spend an evening on this. Most free top-down
packs are drawn **straight down or in a shallow 3/4**, and this game is a 2:1
dimetric isometric — the ground plane recedes at 26.6°. A pack that does not
share that angle will read as characters standing *on* the world rather than
*in* it, and no import flag fixes that; it is a property of the drawings. Import
one character first, look at it next to an unreplaced one, and decide there.
Packs that say "isometric" in the title are the ones that drop in cleanly.

> **Licensing.** Everything currently in this repo is generated by code in this
> repo, which is why it can be AGPL end to end. A downloaded pack is not: it
> comes with its own licence, and most free packs are CC0, CC-BY or "free with
> credit". If you ship one, record the pack, its author, its licence and its URL
> in a `LICENSES.md` next to the `LICENSE`, and drop the "all art is original"
> claim at the top of this file. CC-BY without the attribution is not free art,
> it is just an unattributed copy.
